...
from src.processing import calculate_similarity_memory_efficient, process_grouped_results
...
from src.cache import load_uploaded_file, get_cleaned_df, get_vectorizer_and_vectors
from src.results import (
    _sanitize_for_streamlit,
    _get_process_memory_mb,
    update_results_with_additional_columns,
    convert_streaming_results_to_dataframe,
)
...
def main():
    ...
    if catalog_file and (customer_file or settings['matching_mode'] == "Find Similar Within File"):
        try:
            # Load dataframes (cached so reruns are fast)
            catalog_df = load_uploaded_file(catalog_file)
            ...
            if settings['matching_mode'] == "Find Similar Within File":
                ...
                customer_df = load_uploaded_file(customer_file)
            ...
            if st.button("✨ Find Matches"):
                ...
                with st.spinner("Processing... this may take a moment for large files."):
                    ...
                    # --- Data Cleaning (cached) ---
                    cleaned_catalog_df = get_cleaned_df(
                        catalog_df, column_config['catalog'],
                        settings['remove_stop_words'], settings['case_sensitive'], settings['include_size_in_text']
                    )
                    cleaned_customer_df = get_cleaned_df(
                        customer_df, column_config['customer'],
                        settings['remove_stop_words'], settings['case_sensitive'], settings['include_size_in_text']
                    )
                    ...
                    _, catalog_vectors, customer_vectors = get_vectorizer_and_vectors(
                        cleaned_catalog_df, cleaned_customer_df, settings['remove_stop_words'], settings['enable_text_matching']
                    )
